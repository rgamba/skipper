...
import com.maestroworkflow.api.DecisionRequest;
import com.maestroworkflow.api.DecisionResponse;
...
public class DecisionExecutor {
    ...
    public DecisionResponse execute(@NonNull DecisionRequest decisionRequest, @NonNull Injector injector) {
        DecisionResponse.DecisionResponseBuilder builder = DecisionResponse.builder().operationRequests(new ArrayList<>());
        try {
            ...
            val reqs = e.getOperationRequests().stream()
                    .map(req -> req.toBuilder().workflowInstanceId(decisionRequest.getWorkflowInstance().getId()).build())
    ...
    @SneakyThrows
    public DecisionResponse.DecisionResponseBuilder executeInternal(@NonNull DecisionRequest decisionRequest,
        ...
        val params = inspector.getWorkflowMethodParams(decisionRequest.getWorkflowInstance().getInitialArgs());
        Object result = null;
        try {
            result = workflowMethod.invoke(decider, params);
        } catch (IllegalAccessException e) {
            throw new IllegalStateException(String.format("unable to invoke workflow method on workflow %s", clazz), e);
        } catch (InvocationTargetException e) {
            if (e.getCause() != null && e.getCause() instanceof StopWorkflowExecution) {
                throw ((StopWorkflowExecution) e.getCause()).toBuilder().newState(inspector.getState()).build();
            }
            if (e.getCause() != null) {
                throw e.getCause();
            }
    ...
    @SneakyThrows
    public Map<String, Anything> executeSignalConsumer(@NonNull WorkflowInstance instance, @NonNull Injector injector,
                                                       @NonNull String signalMethodName, @NonNull List<Anything> args) {
        ...
        try {
            ...
            throw new IllegalStateException(String.format("unable to invoke signal consumer method '%s' on workflow %s",
                    signalMethodName, clazz), e);
        } catch (InvocationTargetException e) {
            if (e.getCause() != null) {
                throw e.getCause();
            }
