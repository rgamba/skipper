...
public class OperationProxyFactory {
    ...
    public static <T> T create(Class<T> clazz, @Nullable OperationConfig config) {
        ...
        try {
            instance = factory.create(new Class<?>[0], new Object[0], new OperationProxyHandler(WorkflowContext.get().getOperationResponses(), WorkflowContext.get(), clazz, config, isWorkflow));
        } catch (Exception e) {
