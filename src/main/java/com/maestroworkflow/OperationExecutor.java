...
public class OperationExecutor {
    ...
    public static RetryStrategy DEFAULT_RETRY_STRATEGY = FixedRetryStrategy.builder().retryDelay(Duration.ofSeconds(5)).maxRetries(5).build();
    public static OperationConfig DEFAULT_OPERATION_CONFIG = OperationConfig.builder().retryStrategy(DEFAULT_RETRY_STRATEGY).build();

    public OperationExecutionResponse execute(@NonNull OperationRequest request, @NonNull Injector injector) {
        val responseBuilder = OperationExecutionResponse.builder();
        if (request.getOperationType().isWaitTimeout()) {
            return responseBuilder.status(OperationExecutionResponse.Status.NON_RETRIABLE_ERROR)
        ...
        try {
            ...
            Class<? extends Throwable> clazz = e.getCause() != null ? e.getCause().getClass() : e.getClass();
    ...
    private Method getMethod(OperationType operationType) {
        return Stream.of(operationType.getClazz().getDeclaredMethods())
                ...
                .orElseThrow(() -> new IllegalArgumentException(
                        String.format("the operation class %s does not have a method %s",
