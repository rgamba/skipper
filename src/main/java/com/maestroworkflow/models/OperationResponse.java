...
public class OperationResponse implements Serializable {
    ...
    public OperationResponse(@NonNull String id, @NonNull String workflowInstanceId, @NonNull OperationType operationType,
                             int iteration, @NonNull Instant creationTime, boolean isSuccess, boolean isTransient,
                             @NonNull String operationRequestId, Anything result, Anything error, Duration executionDuration,
                             String childWorkflowInstanceId) {
        ...
        ValidationUtils.when(operationType.isWorkflow())
                .thenExpect(childWorkflowInstanceId != null, "when operation type is workflow, childWorkflowInstanceId must be provided");
        this.id = id;
