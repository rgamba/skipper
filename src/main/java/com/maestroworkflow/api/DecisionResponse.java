...
public class DecisionResponse {
    ...
    public DecisionResponse(Anything result, @NonNull List<OperationRequest> operationRequests, Map<String, Anything> newState, @NonNull WorkflowInstance.Status newStatus, String statusReason, Duration waitForDuration) {
        ValidationUtils.when(newStatus.equals(WorkflowInstance.Status.COMPLETED) || newStatus.equals(WorkflowInstance.Status.ERROR))
                .thenExpect(operationRequests.isEmpty(), "operationRequests are not allowed when workflow instance status is %s", newStatus);
        ...
        ValidationUtils.when(newStatus.equals(WorkflowInstance.Status.ACTIVE))
                .thenExpect(operationRequests.size() > 0 || waitForDuration != null, "operationRequests or waitDuration are expected when Status == ACTIVE");
        ValidationUtils.when(waitForDuration != null)
                .thenExpect(operationRequests.isEmpty(), "cannot have a simultaneous wait and operation request");
